...
void tampilkanTabelKonserRekursif(const Konser konser[], int idx, int jumlah) {
    ...
    printf("| %2d | %-20s | %-12s | %-15s | %8s |\n",
        idx+1, k.judul.c_str(), k.lokasi.c_str(), k.tanggal.c_str(), k.harga.c_str());
...
void hapusKonser(Konser konser[], int &jumlah) {
    ...
    jumlah--; tampilkanPesan("Konser berhasil dihapus.");
}
...
void lihatBooking(const Pengguna &u) {
    ...
    for(int i=0;i<u.jumlahBooking;i++){
        ...
        printf("| %2d | %-20s | %-12s | %-15s | %8s |\n",
          i+1, k.judul.c_str(), k.lokasi.c_str(), k.tanggal.c_str(), k.harga.c_str());
    }
    cout << "+----+----------------------+--------------+-----------------+----------+\n";
}
...
int main(){
    ...
    while(true){
        ...
        int opsi; cin>>opsi;
        if(opsi==1) registrasi(pengguna, jumlahUser);
        else if(opsi==2){
