...
// Fungsi untuk memeriksa apakah angka adalah bilangan palindrome
...
// Fungsi login, jika salah user harus login kembali
bool login() {
    string namaPengguna, namaBenar = "Ikhsan";
    string kataSandi, sandiBenar = "2409106118";

    cin.ignore(); // Membersihkan buffer input sebelum input string
    while (true) {
        cout << "===== LOGIN =====\n";
        cout << "Nama Pengguna: ";
...
int main() {
    while (true) {
        ...
        // Memastikan login berhasil sebelum melanjutkan ke menu
        if (!login()) {
            continue;
        }
        ...
        while (true) {
            ...
            while (true) {
                ...
                if (pilihan == 1) {
                    ...
                    break; // Kembali ke input kode sandi
                } 
                else if (pilihan == 5) {
